  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.status === 409) {
        ...
        // Registration succeeded (201) — automatically log in with same credentials
        try {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-buddy-base p-8">
      <div className="w-full max-w-[420px] px-2">
        ...
        <h1 className="text-xl font-semibold text-buddy-text">Utwórz konto</h1>
        <p className="text-sm text-buddy-text-muted mt-1 mb-8">
          Zarejestruj się, aby korzystać z AI Buddy
        </p>
        ...
        <p className="text-sm text-buddy-text-muted mt-6 text-center">
          Masz już konto?{" "}
          <Link href="/login" className="cursor-pointer text-buddy-gold hover:text-buddy-gold-light underline-offset-2 hover:underline">
            Zaloguj się
          </Link>
        </p>
      </div>
